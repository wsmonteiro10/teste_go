...
func SalvarLotesfast(records [][]any) error {
	chunks := SepararEmLotesFast(records, batchSize)
	...
	for w := 0; w < maxWorkers; w++ {
		...
		go func() {
			...
			for batch := range jobs {
				if err := InserirEmLoteFast(batch); err != nil {
					errs <- err
				}
			}
		}()
...
func InserirEmLoteFast(records [][]any) error {
	if len(records) == 0 {
		return nil
	}
	...
	b.WriteString(`
		INSERT INTO cliente_compras_noindex (
...
func SepararEmLotesFast(data [][]any, size int) [][][]any {
	var chunks [][][]any
	for size < len(data) {
